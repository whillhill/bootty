...
def copy_binary(dist_root: Path, platform: dict[str, str]) -> None:
    ...
    module = platform["python_module"]
    target_dir = ROOT / "python" / "packages" / platform["python_package"] / "src" / module / "bin"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Assemble dist artifacts into PyPI platform packages")
